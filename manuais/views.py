...
class ManuaisNewView(CreateView):
	...
	def get(self, request, *args, **kwargs):    
		# equipamento = self.nome_equipamento
		...
		# if request.nome_equipamento.is_valid():
		# manuais = RevisaoManuais.objects.filter(nome_equipamento=equipamento)
		# manuais = Equipamentos.objects.filter(nome_equipamento=equipamento)
		# form.field['nome_equipamento'].queryset = manuais
		# form.field['nome_equipamento'] = manuais 
		context = {
			'form': form,
			'carro': equipamento
		}
	...
	def post(self, request, *args, **kwargs):
		...
		if 'btn_adicionar' in self.request.POST:
			# if Equipamentos.objects.filter(nome_equipamento=equipamento).filter(
			# 		nome_equipamento=nome_equipamento).exists():
			# 	context = {'form': form}
			...
			# 	context = {'form': form}
			# 	return render(request, 'manuais/manual-novo.html', context, kwargs={'pk': self.object.pk, })
